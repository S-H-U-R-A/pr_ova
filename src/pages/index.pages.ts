export { LoginPage }            from '../pages/Auth/login/login';
export { RegisterPage }         from '../pages/Auth/register/register';
export { HomeTeacherPage }      from '../pages/teacher/home-teacher/home-teacher';
export { HomeStudentPage }      from '../pages/student/home-student/home-student';
export { TeacherProblemPage }   from '../pages/teacher/teacher-problem/teacher-problem';
export { StudentProblemPage }   from '../pages/student/student-problem/student-problem';
export { ProfilePage }          from '../pages/profile/profile';
export { ModalAvatarPage }      from '../pages/Auth/modal-avatar/modal-avatar';
export { ModalPhotoPage }       from '../pages/modal-photo/modal-photo';
export { ModalUpdateUserPage }  from '../pages/modal-update-user/modal-update-user';
export { ModalRespuestaPage }   from '../pages/teacher/modal-respuesta/modal-respuesta';
export { ModalSugerenciaPage }  from '../pages/teacher/modal-sugerencia/modal-sugerencia';
export { StudentProblemPageOnePage }    from '../pages/student/student-problem-page-one/student-problem-page-one';
export { PopoverSuggestionPage }        from '../pages/student/popover-suggestion/popover-suggestion';
export { StudentProblemTwoPage }        from '../pages/student/student-problem-two/student-problem-two';
export { MostrarInformePage }           from '../pages/teacher/mostrar-informe/mostrar-informe';
export { DetalleInformePage }           from '../pages/teacher/detalle-informe/detalle-informe';